    ...
    h('div', { className: 'dialog', style: 'max-width:360px;max-height:80vh;overflow-y:auto' }, [
      ...
      h('div', { style: 'margin-top:12px;text-align:center' }, [
        h('button', {
          ...
          onclick: async function() { overlay.remove(); await reloadFn(); if (completeFn) await completeFn(lastAddedName); }
        }, '完成')
      ])
    ])
  ]);
...
// newCategoryName: 若传入则选中该分类（用户刚在管理弹窗中新增的分类），否则保留当前选中值
function refreshCategorySelect(newCategoryName) {
  const catSelect = document.getElementById('edit-category');
  if (!catSelect) return;
  const currentValue = newCategoryName || catSelect.value;
  catSelect.innerHTML = '';
